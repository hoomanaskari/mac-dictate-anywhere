...
│   ├── ModelCardView.swift         # Model display component
...
│   └── OverlayWindow/
│       ├── AudioWaveformView.swift      # Animated waveform visualization
│       ├── OverlayContentView.swift     # Overlay UI states
...
|---------|----------------|-------------|
| `TranscriptionService` | WhisperKit lifecycle, audio recording | `startRecording()`, `stopRecording()` |
...
### Thread Safety
Use locks for concurrent state access:
```swift
...
### Thread Safety
- Recording state uses `NSLock` to prevent concurrent access
