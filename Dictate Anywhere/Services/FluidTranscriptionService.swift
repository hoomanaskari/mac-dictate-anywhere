...
private enum AsrManagerCoordinatorError: Error {
    case notInitialized
}

/// Serializes access to AsrManager to prevent concurrent CoreML usage.
private actor AsrManagerCoordinator {
    ...
    func initialize(models: AsrModels, config: ASRConfig) async throws {
        manager?.cleanup()
        let newManager = AsrManager(config: config)
        try await newManager.initialize(models: models)
        manager = newManager
    }

    func transcribe(_ audioSamples: [Float]) async throws -> ASRResult {
        guard let manager else { throw AsrManagerCoordinatorError.notInitialized }
        return try await manager.transcribe(audioSamples)
    }
...
final class FluidTranscriptionService {
    ...
    private let asrManagerCoordinator = AsrManagerCoordinator()
    ...
    func initialize(with models: AsrModels) async throws {
        ...
        try await asrManagerCoordinator.initialize(models: models, config: asrConfig)
    ...
    func startRecording(deviceID: AudioDeviceID? = nil, startTimeout: TimeInterval = 0.75) async -> RecordingStartOutcome {
        ...
        guard await asrManagerCoordinator.isInitialized() else {
    ...
    func stopRecording() async -> String {
        ...
        // Stop the transcription loop and wait for in-flight ASR calls to finish.
        await stopTranscriptionLoop()
    ...
    private func stopTranscriptionLoop() async {
    ...
    private func transcriptionLoop() async {
        guard await asrManagerCoordinator.isInitialized() else { return }
        ...
        while isTranscribing && !Task.isCancelled {
            ...
            if hasSignificant {
                ...
                do {
                    let result = try await asrManagerCoordinator.transcribe(samplesArray)
    ...
    private func performFinalTranscription() async -> String {
        ...
        do {
            let result = try await asrManagerCoordinator.transcribe(audioSamples)
            let text = result.text.trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    func cleanup() async {
        ...
        await asrManagerCoordinator.cleanup()
        loadedModels = nil
        isModelLoaded = false
    }
